...
/**
 * @brief For generating the random coordinate
 * 
 * @param k_cluster  total cluster that we have
 * @return int*  the inital value of cluster
 */
int* generate_random_coordinate(int k_cluster){
  int i;
  int *coordinate = malloc(sizeof(int) * k_cluster * 2);
  for(i = 0; i < k_cluster; i++){
    coordinate[i*2] = rand() % (rows);
    coordinate[i*2 + 1] = rand() % (cols);
  }
 ...
 * @param coordinate  cluster coordinate
 * @param row  row of the pixel
 * @param col  col of the pixel
 * @return int  the nearest cluster 
 */
int euclidean_distance(gray *pixel, int k_cluster, int *cluster, int *coordinate, int row, int col){
  ...
  for(i = 0; i < k_cluster; i++){
    ...
    temp += (row - coordinate[i * 2]) * (row - coordinate[i * 2]);
    temp += (col - coordinate[i * 2 + 1]) * (col - coordinate[i * 2 + 1]);
 ...
 * @param k_cluster  total cluster that we have
 * @param cluster  value of mark/label that we will use
 * @param coordinate  value of coordinate cluster that we will use
 * @return int*  the new label/mark
 */
int* map(int k_cluster, int *cluster, int *coordinate){
  int i, j;
  int *temp = malloc(sizeof(int) * cols * rows);
  for(i = 0; i < cols; i++){
    for(j = 0; j < rows; j++){
      int index = i * (rows * pixel_val) + (pixel_val * j);
      temp[i * rows + j] = euclidean_distance(&image[index], k_cluster, cluster, coordinate, j, i);
    }
  }
...
int *update_cluster(int k_cluster, int *mark){
  int *cluster = malloc(sizeof(int) * k_cluster * pixel_val);
  int *temp = malloc(sizeof(int) * k_cluster * pixel_val);
...
/**
 * @brief For Update coordinate based on the value of the mark
 * 
 * @param k_cluster  total cluster that we have
 * @param mark  value of mark/label that we will use
 * @return int*  the new cluster coordinate
 */
int *update_coordinate(int k_cluster, int *mark){
  int *coordinate = malloc(sizeof(int) * k_cluster * 2);
  long int *temp = malloc(sizeof(long int) * k_cluster * 2);
  int *count = malloc(sizeof(int) * k_cluster);
  ...
  for(i=0; i < k_cluster; i++){
    for(j=0; j<2; j++){
      if(count[i] != 0)
        coordinate[i*2 + j] = (int) (temp[i * 2 + j] / count[i]);
    }
  }
...
int main(int argc, char* argv[]){
  ...
  */
  ...
  coordinate = generate_random_coordinate(k_cluster);
  ...
  // cluster[7] = 151;
  // cluster[8] = 237;
  int *start_cluster = copy(cluster, k_cluster);
  int *start_coordinate = copy(coordinate, k_cluster);

  for(i=0; i < total_runs; i++){
    mark = map(k_cluster, cluster, coordinate);
    cluster = update_cluster(k_cluster, mark);
    coordinate = update_coordinate(k_cluster, mark);
  ...
  printf("\n");
  print(k_cluster, start_cluster, mark);
  for(i = 0; i < k_cluster; i++){
    printf("%d %d\n", start_coordinate[i*2], start_coordinate[i*2+1]);
  }
  printf("\n");
  print(k_cluster, cluster, mark);
  for(i = 0; i < k_cluster; i++){
    printf("%d %d\n", coordinate[i*2], coordinate[i*2+1]);
  }
