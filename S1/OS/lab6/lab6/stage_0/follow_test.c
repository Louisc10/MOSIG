...
{
    printf("Usage: %s -m hostname -p port_number -t nb_timeline_requests -k max_nb_publish -s [activate_streaming]\n", exec);
...
static void *publish_thread (void *arg)
{
    ...
    while(!__sync_bool_compare_and_swap(&keep_on_going, 0, 2) && publish_count < max_publish){
        memset(my_msg, 0, BABBLE_PUB_SIZE);
        snprintf(my_msg, BABBLE_PUB_SIZE, "msg_%d", publish_count);
    ...
    /* synch with server -- required with streaming */
    if(with_streaming){
...
static void *follow_thread (void *arg)
{
    ...
    for(i=0; i< nb_timeline; i++){
        ...
        if(timeline_size == -1){
            ...
            if(timeline_size < 5){
                /* leave some time to the publisher thread to do some
                   work */
                usleep(1000);
            }
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt (argc, argv, "+hm:p:t:k:s")) != -1){
        switch (opt){
        case 'm':
