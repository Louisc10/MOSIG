...
typedef struct client_thread_data{
    ...
    pthread_barrier_t *gbarrier_half;
...
static void *client_thread (void *arg)
{
    ...
    char my_msg[BABBLE_PUB_SIZE];
    for(i=0; i< data->nb_msgs; i++){
        memset(my_msg, 0, BABBLE_PUB_SIZE);
        snprintf(my_msg, BABBLE_PUB_SIZE, "ping_%d:%d", data->client_id, i);
    ...
    /* global barrier before disconnecting half the clients */
    ret = pthread_barrier_wait(data->gbarrier);
    if (ret != 0 && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        fprintf(stderr, "Barrier synchronization failed!\n");
    ...
    if(data->client_id %2 == 0){
        close(sockfd);
        return (void*)EXIT_SUCCESS;
    }
    
    /* publishing k additional msgs */
    for(i=0; i< data->nb_msgs; i++){
        memset(my_msg, 0, BABBLE_PUB_SIZE);
        snprintf(my_msg, BABBLE_PUB_SIZE, "ping_%d:%d", data->client_id, i+ data->nb_msgs);
        if(client_publish(sockfd, my_msg, with_streaming)){
            fprintf(stderr,"*** Test Failed ***\n");
            fprintf(stderr,"%s failed to publish %s\n", client_name, my_msg);
            close(sockfd);
    ...
    /* global barrier before asking timelines */
    ret = pthread_barrier_wait(data->gbarrier_half);
    if (ret != 0 && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        fprintf(stderr, "Barrier synchronization failed!\n");
        return (void*)EXIT_FAILURE;
    ...
    if(timeline_size != data->nb_clients/2 * data->nb_msgs){
        fprintf(stderr,"*** Test Failed ***\n");
        fprintf(stderr,"%s has %d msgs in timeline (%d expected)\n", client_name, timeline_size, data->nb_clients * data->nb_msgs);
        close(sockfd);
        exit(-1);
    ...
    nb_followers = client_follow_count(sockfd);
    if(nb_followers != data->nb_clients){
        fprintf(stderr,"*** Test Failed ***\n");
        fprintf(stderr,"%s has  %d followers\n", client_name, nb_followers);
        close(sockfd);
    ...
    /* global barrier before terminating */
    ret = pthread_barrier_wait(data->gbarrier);
    if (ret != 0 && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        fprintf(stderr, "Barrier synchronization failed!\n");
...
static void *client_thread_new (void *arg)
{
...
int main(int argc, char *argv[])
{
    ...
    pthread_barrier_t global_barrier_half;
    ...
    if( nb_threads == -1 || nb_msgs == -1){
        ...
        if(nb_threads < 2){
            printf("Error: number of clients (-n) should be at least 2\n");
            return -1;
        }
        else{
            printf("starting stress test with %d clients sending %d msgs\n", nb_threads, nb_msgs);
            nb_threads_renewed = (nb_threads/2) + (nb_threads%2);
        }
    }
    ...
    if(pthread_barrier_init(&global_barrier_half, NULL, nb_threads/2+1))
    {
        printf("Could not create a barrier\n");
        return -1;
    }
    ...
    tids = malloc(sizeof(pthread_t) * (nb_threads + nb_threads_renewed));
    clients_data = malloc(sizeof(client_thread_data_t) * (nb_threads + nb_threads_renewed));
    
    for(i=0; i < nb_threads; i++){
        clients_data[i].gbarrier= &global_barrier;
        clients_data[i].gbarrier_half= &global_barrier_half;
    ...
    /* global barrier before disconnecting half of the clients */
    ret = pthread_barrier_wait(&global_barrier);
    if (ret != 0 && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        fprintf(stderr, "Barrier synchronization failed!\n");
    ...
    printf("**** Info: disconnecting half of the clients and connecting new clients\n");

    for(i=0; i < nb_threads_renewed; i++){
        int id_renewed = nb_threads + i;
        clients_data[id_renewed].gbarrier= &global_barrier;
        clients_data[id_renewed].gbarrier_half= &global_barrier_half;
        clients_data[id_renewed].nb_msgs = nb_msgs;
        clients_data[id_renewed].nb_clients = nb_threads;
        clients_data[id_renewed].client_id = id_renewed;
        if(pthread_create (&tids[id_renewed], NULL, client_thread_new, (void*) &clients_data[id_renewed]) != 0){
            fprintf(stderr,"WARNING: Failed to create comm thread\n");
        }
        /* we create the threads by groups of 50 to reduce the stress
           during the registration */
    ...
    /* global barrier before before asking timelines */
    ret = pthread_barrier_wait(&global_barrier_half);
    if (ret != 0 && ret != PTHREAD_BARRIER_SERIAL_THREAD)
    {
        fprintf(stderr, "Barrier synchronization failed!\n");
        return -1;
    ...
    for(i=0; i < nb_threads + nb_threads_renewed; i++){
